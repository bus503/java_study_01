...
public class Exam02 {
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(stdArr));  //참조타입이라 초기값은 전부 null
		initArr(stdArr);
		...
		findStd.setStdNo(15);
		idx = indexOf(stdArr, findStd); // 존재하지 않는 학생이므로 -1 예상
		if(idx == -1 ) {
	...
	private static void initArr(Student[] arr) {
		...
		for(int i=0; i<arr.length; i++) {
			arr[i] = new Student(i+1, names[i], rnd.nextInt(60)+41,
												rnd.nextInt(60)+41,
												rnd.nextInt(60)+41);
		}
	}
